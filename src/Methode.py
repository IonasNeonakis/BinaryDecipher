...
class Methode():
    ...
    def evaluate(self, doitPrint):
        ...
        for instruction_offset in self._succ.keys():
            self._tmp_map_register[instruction_offset] = copy.deepcopy(self._etat_reg)
        ...
        while len(to_do) > 0:
            ...
            if len(parents) > 1:
                ...
                registres_parent = self._tmp_map_register.get(offset_parent)
                register_after_merge = copy.deepcopy(registres_parent)

                for i in range(1, len(parents)):
                    instr_parentI, _, offset_parentI = parents[i]
                    registres_parentI = self._tmp_map_register.get(offset_parentI)
                        ...
                        if type:
                            if type[0] == 'L' or type[0][0] == 'L':
                                ...
                                if registres_parent.get(num)[0] == type:
                                    ...
                                    self.set_verbose_error(curr_instr, f"Erreur de merge a l'instruction {curr_instr.get_name()} : les parents {parents[0]} et {parents[i]} ne sont pas merge friendly. (OBJECT)")
                                    pass #Todo
                            else:
                                ...
                                if not registres_parent.get(num)[0]:
                                    ...
                                    self.set_verbose_error(curr_instr, f"Erreur de merge a l'instruction {curr_instr.get_name()} : les parents {parents[0]} et {parents[i]} ne sont pas merge friendly.")
                        else:
                            # Le type est None, alors on garde le p1
                            register_after_merge[num] = registres_parent.get(num)
                if self.register_equals(self._tmp_map_register.get(offset), register_after_merge):
                    to_do.pop(0)
                    continue
                else:
            ...
            if self.check_registers_accessibility(curr_instr.get_register()):
                if curr_instr.get_name()[:4] == "move":
                    if curr_instr.get_name() in ['move', 'move-wide', 'move-object', 'move/from16', 'move-wide/from16',
                        ...
                        if 'wide' in curr_instr.get_name():
                            if curr_instr.get_register()[0] + 1 in self._etat_reg.keys():
                                ...
                                self.set_verbose_error(curr_instr, f"Erreur à l'instruction")
                                is_valide = False
                        else:
                    ...
                    if not self._informations['return'] == 'void':
                        self.set_verbose_error(curr_instr, 'Erreur dans les registres(méthode :  ' + curr_instr.get_name() + ', le type de retour ne correspond pas.)')
                    ...
                    if not self._informations['return'] == self._etat_reg[curr_instr.get_register()[0]][0]:
                        self.set_verbose_error(curr_instr, "Erreur de type de retour")
                        is_valide = False
                    ...
                    if not (curr_instr.get_register()[0] + 1 in self._etat_reg.keys() and self._etat_reg.get(curr_instr.get_register()[0])[0] == self._informations['return'] and self._etat_reg.get(curr_instr.get_register()[0] + 1)[0] == self._informations['return']):
                        self.set_verbose_error(curr_instr, f"Erreur à l'instruction {curr_instr.get_name()}")
                        is_valide = False
                    ...
                    if self._etat_reg[tab[1]][0] != 'int' or self._etat_reg[tab[2]][0] != 'int':
                        self.set_verbose_error(curr_instr, 'Erreur dans les registres, ce ne sont pas des int')
                    ...
                    if self._etat_reg[tab[1]][0] not in ["int", "float", "short", "double", "long"] or self._etat_reg[tab[2]][0] not in ["int", "float", "short", "double", "long"]:
                        self.set_verbose_error(curr_instr,
                                               'Erreur dans les registres(méthode :  ' + curr_instr.get_name() + ', ce ne sont pas des ' + type + ' )\033[0m')
                        is_valide = False
                    ...
                    if self._etat_reg[tab[1]][0] != 'int':
                        self.set_verbose_error(curr_instr, 'Erreur dans les registres(méthode :  ' + curr_instr.get_name() + ', ce ne sont pas des int)')
                        is_valide = False
                    self._etat_reg[tab[0]] = ('int', None)
                    ...
                    if self._etat_reg[tab[0]][0] != type or self._etat_reg[tab[1]][0] != type:
                        self.set_verbose_error(curr_instr, 'Erreur dans les registres(méthode :  ' + curr_instr.get_name() + ', ce ne sont pas des ' + type)
                        # return False
                    ...
                    if len(name) == 2:  # si on est là ,if-eq,if-ne,if-lt,if-ge,if-gt,if-le
                        if self._etat_reg[curr_instr.get_register()[0]][0] != self._etat_reg[curr_instr.get_register()[1]][0]:
                            self.set_verbose_error(curr_instr, 'Erreur dans les registres(méthode :  ' + curr_instr.get_name() + ',test d\'egalite sur des types differents)')
                    else:  # si on est là ,if-eqz,if-nez,if-ltz,if-gez,if-gtz,if-lez
                        if self._etat_reg[curr_instr.get_register()[0]][0] == 'None':
                            self.set_verbose_error(curr_instr, 'Erreur dans les registres(méthode :  ' + curr_instr.get_name() + ', le type \'None\' n\'est pas comparable avec 0.)')
                            # return False
                    ...
                    self.set_verbose_error(curr_instr, curr_instr.get_name() + " n'est pas encore pris en compte dans Methode.py" + '\033[0m\n')

            else:
                self.set_verbose_error(curr_instr, f"Erreur dans les registres (méthode : {curr_instr.get_name()}, la méthode accède à des registres inaccessibles)")
            self._tmp_map_register[offset] = copy.deepcopy(self._etat_reg) #Fixme
            to_do.pop(0)
    ...
    def print(self):
        ...
        return f"Méthode {self.get_androguard_method().name}{self.get_androguard_method().proto} (\n    Informations : {self._informations} \n    Nombre de registres : {self._nb_reg}\n    Instructions : \n{instructions_pretty_print})"
